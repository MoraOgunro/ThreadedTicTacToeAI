...
public class MinMax {
	...
	private edu.mogunr2.project4.Node initNode;			//node for the initial state
	
	private ArrayList<edu.mogunr2.project4.Node> stateList;	//a list to contain all possible moves for X
	 ...
	 * @return
	 */
	ArrayList<edu.mogunr2.project4.Node> findMoves()
	{		
		for(int x = 0; x < stateList.size(); x++) 				//for each node in the state list, find it's min/max number
		{
			edu.mogunr2.project4.Node temp = stateList.get(x);					//get node off list
	...
	{
		initNode = new edu.mogunr2.project4.Node(initState, 0);
	}
	 ...
	 */
	private ArrayList<edu.mogunr2.project4.Node> createStateList(String minOrMax, edu.mogunr2.project4.Node stateNode)
	{
		
		ArrayList<edu.mogunr2.project4.Node> stateList = new ArrayList<edu.mogunr2.project4.Node>();		//array list to hold all possible moves
		...
		if(minOrMax == "max")		//list of all X moves
		{
			...
			for(int x = 0 ; x < initState.length ; x++)
			{
				if(initState[x].equals("b"))								//its a blank space
				{
					...
					edu.mogunr2.project4.Node temp = new edu.mogunr2.project4.Node(copyInit, x+1);				//make a Node out of it
	 ...
	 * @param list
	 */
	void printList(ArrayList<edu.mogunr2.project4.Node> list)
	{
		for(int x = 0; x < list.size(); x++ )		//get a node from the list
		{
			edu.mogunr2.project4.Node temp = list.get(x);
	 ...
	 * @param x_or_y
	 */
	void setStateList_MinMaxValues(ArrayList<edu.mogunr2.project4.Node> theStateList, int x_or_y)
	{
		if(x_or_y == 1)						//set the minMax values for X
		{
			for(int x =0; x < theStateList.size(); x++)
			{
				edu.mogunr2.project4.Node temp = theStateList.get(x);
	 ...
	 * @param list
	 */
	void print_minMax(ArrayList<edu.mogunr2.project4.Node> list)
	{
	 ...
	 * @return
	 */
	int Min(edu.mogunr2.project4.Node state)
	{
		boolean is_a_win = false, is_a_tie = false;
		
		//create list of possible moves
		ArrayList<edu.mogunr2.project4.Node> listOfStates = createStateList("min", state);
		...
		{
			edu.mogunr2.project4.Node temp = listOfStates.get(0);
			return temp.getMinMax();
		}
		//if the list has a -10 return -10
		for(int x = 0; x < listOfStates.size(); x++)
		{
			edu.mogunr2.project4.Node temp = listOfStates.get(x);
		...
		{
			edu.mogunr2.project4.Node temp = listOfStates.get(y);					//get node off list
		...
		{
			edu.mogunr2.project4.Node temp = listOfStates.get(i);
	 ...
	 * @return
	 */
	int Max(edu.mogunr2.project4.Node state)
	{
		
		boolean is_a_win = false, is_a_tie = false;
		
		//create list of possible moves
		ArrayList<edu.mogunr2.project4.Node> listOfStates = createStateList("max", state);
